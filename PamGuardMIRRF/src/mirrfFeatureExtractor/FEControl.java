...
public class FEControl extends MIRRFControlledUnit implements PamSettings {
	...
	/**
	 * Searches for feature names within an input .mirrffe or .mirrfts file.
	...
	public static ArrayList<String> findFeaturesInFile(File f) {
		if (!(f.getPath().endsWith(".mirrffe") || f.getPath().endsWith(".mirrfts"))) return null;
		...
		try {
			...
			while (sc.hasNextLine()) {
				String next = sc.nextLine();
				if ((f.getPath().endsWith(".mirrffe") && next.startsWith("cluster,uid,date,duration,lf,hf,")) ||
						(f.getPath().endsWith(".mirrfts") && next.startsWith("cluster,uid,location,date,duration,lf,hf,label,"))) {
					String[] tokens = next.split(",");
					int startindex = 6;
					if (f.getPath().endsWith(".mirrfts")) startindex = 8;
	...
	/**
	 * The same as findFeaturesInFile, but only includes header features that can't be calculated if they otherwise weren't present in the input .mirrfts file:
	 * amplitude, freqsd, freqsdd1, freqsdd2, freqsdslope, freqsdelbow
	 */
	 ...
	 * Files must be either .mirrffe or .mirffts.
